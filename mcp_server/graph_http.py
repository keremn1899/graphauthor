...
def graph_routes(catalogue: GraphCatalogue, *, current_surface: Any | None = None,
    ...
    async def what_governs(request):
        try:
            body = await request.json()
        except Exception:
            body = {}
        ...
        if not question:
            return json_result(
                operator_fault("invalid", "Ask a governance question first.")
            )
        ...
        try:
            ...
            return json_result(
                operator_fault(
                    "unavailable",
                    f"what_governs failed: {type(exc).__name__}: {exc}",
                )
            )

    async def check_conformance(request):
        try:
            body = await request.json()
        except Exception:
            body = {}
        ...
        if not any(
            ...
            for key in ("artifact", "artifact_path", "diff", "receipt")
        ):
            return json_result(
                operator_fault(
                    "invalid",
                    "Provide artifact, artifact_path, diff, or receipt.",
                )
            )
        allowed = {
            "rule_id",
        ...
        try:
            ...
            result = await run_in_threadpool(
                _surface_call, path, "check_conformance", **kwargs
            )
            return JSONResponse(result)
        except Exception as exc:
            return json_result(
                operator_fault(
                    "unavailable",
                    f"check_conformance failed: {type(exc).__name__}: {exc}",
                )
            )
    ...
    async def discover(request):
        ...
        if not query:
            return json_result(operator_fault("invalid", "Ask a question first."))
        path = catalogue.resolve(str(body.get("graph_id", "")) or None)
        if path is None:
            return json_result(operator_fault("not_found", "unknown graph"))

        def run_query():
            return _surface_call(
                path, "discover", query, evidence="packet", explain=True
            )

        try:
            from starlette.concurrency import run_in_threadpool

            result = await run_in_threadpool(run_query)
            # The overlay is a deterministic projection of what came back —
            # which nodes the answer stood on, and which of those sit next to
            # material it did not use. Attached here rather than fetched
            # separately: it needs no new authority and no second traversal.
            # Ids and roles only; an overlay can never move a node.
            if isinstance(result, dict):
                result["overlay"] = await run_in_threadpool(
                    _overlay_for, path, result)
            return JSONResponse(result)
        except Exception as exc:
            # Queries may depend on model/provider configuration. Keep that
            # operational failure typed and local rather than returning an HTML
            # traceback that the product can only call "500".
            return json_result(
                operator_fault(
                    "unavailable",
                    f"discover failed: {type(exc).__name__}: {exc}",
                )
            )
    ...
    return [
        ...
        Route("/what-governs", what_governs, methods=["POST"]),
        Route("/check-conformance", check_conformance, methods=["POST"]),
    ]
