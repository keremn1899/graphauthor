...
def reject_proposal(
    ...
    *,
    ...
    try:
        ...
        if rec is None:
            return {"error": f"unknown proposal: {proposal_id}"}
        # Terminal states are terminal. Rejecting a COMMITTED proposal used to
        # succeed, leaving the record saying REJECTED while its encoding was
        # live in the graph — an audit trail contradicting the graph it
        # describes, in a product whose value is attributable provenance. A
        # committed change is undone by an operator revert, not by relabelling
        # the proposal that made it.
        #
        # Only COMMITTED and REJECTED are terminal. A gate-failed proposal was
        # restored, so nothing of it is in the graph, and dropping it instead
        # of requeueing it is the operator's ordinary choice — an earlier
        # version of this guard allowed only PENDING and broke exactly that.
        if rec["status"] in ("COMMITTED", "REJECTED"):
            return {"error": f"cannot reject from {rec['status']}"}
        store.update_proposal(
            proposal_id,
            status="REJECTED",
...
def requeue_proposal(
    store_path: Path | str,
    proposal_id: str,
    *,
    actor: str = "operator",
) -> dict[str, Any]:
    """OPERATOR ONLY. A GATE_FAILED / ENCODE_FAILED proposal returns to PENDING
    for re-confirm (e.g. after a gate fix) — audit-appending, never available
    from COMMITTED or REJECTED. Single source of truth for both the CLI and the
    operator BFF: neither carries new authority, both call this."""
    from interaction.write_path_store import WritePathStore
    ...
    try:
        ...
        if rec is None:
            return {"error": f"unknown proposal: {proposal_id}"}
        if rec["status"] not in ("GATE_FAILED", "ENCODE_FAILED", "GRAIN_FAILED"):
            return {"error": f"cannot requeue from {rec['status']}"}
        store.update_proposal(
            proposal_id, status="PENDING",
            demotion_reason=(rec["demotion_reason"] + "; " if rec["demotion_reason"] else "")
            + f"requeued by operator from {rec['status']}")
...
# ---------------------------------------------------------------------------
# Level-1 machinery (write-autonomy-reversal-v1 §2–§4)
#
# Built at M5-prep behind the admission flag: `WritePolicy.l1_admitted` stays
# False until the reversal's §4 checklist is green (live nonce verification +
# real encode adapter). Flipping it is a config change with a paper trail,
# not a build.
# ---------------------------------------------------------------------------
...
@dataclass
class WritePolicy:
    """Per-graph autonomy policy (reversal §2). L0-only by default."""

    l1_admitted: bool = False
    admission_evidence: str = ""  # findings doc / checklist sign-off pointer
    frontier_min_occurrences: int = 3  # mirrors write_path.config default
    legislatable: Callable[[str], bool] | None = None  # domain adapter concern
    # Providers may accept the persisted proposal record. Record-aware gates
    # are required when one long-lived server can auto-commit more than one
    # structural predicate; zero-argument providers remain supported for the
    # original fixed-proposal pilots.
    gate_provider: Callable[..., "GateSpec"] | None = None
    embedder: Callable[[str], list[float]] | None = None
    # Frontier v2 (decision record; option C, STRUCTURAL predicates only).
    # Default OFF: FRONTIER_V2_PASS (incl. one live structural L1 on a
    # development corpus) must go green before any flip. Trust surface is an
    # operator-pinned checkout — never "whatever is on disk".
    frontier_v2_admitted: bool = False
    artifact_root: Path | None = None
    artifact_test_manifest: Path | None = None


def frontier_check(store, predicate: str, policy: WritePolicy) -> tuple[bool, str]:
    """Writable frontier (reversal §3): LEGISLATABLE ∧ ≥N occurrences ∧
    reachable from an existing typed gap. The graph's own gaps define the
    writable frontier — an attacker cannot invent their own hole to fill.

    Occurrences are counted from persisted escalations whose
    ``ungoverned_predicate`` matches; ``legislatable`` is domain-supplied
    (production wiring: write_path.recurrence B-hint typing feeds it).
    """
    predicate = str(predicate or "").strip()
    if not predicate:
        return False, "frontier: proposal carries no target predicate (target_gap_id)"
    occurrences = sum(
        1 for h in store.list_handoffs() if str(getattr(h, "ungoverned_predicate", "")) == predicate
    )
    if occurrences < policy.frontier_min_occurrences:
        return False, (
            f"frontier: predicate {predicate!r} has {occurrences} escalation(s); "
            f"needs ≥{policy.frontier_min_occurrences} — the graph does not yet know this hole"
        )
    if policy.legislatable is None or not policy.legislatable(predicate):
        return False, f"frontier: predicate {predicate!r} is not typed LEGISLATABLE by the domain adapter"
    return True, ""


def _l1_provenance_ok(prov: "ProposalProvenance") -> tuple[bool, str]:
    """L1 requires self-sourcing observation provenance (reversal §2):
    a generating task and at least one source reference."""
    if prov.decision_origin == "propose_new":
        return False, (
            "L1 authority: a genuinely new decision requires human ratification; "
            "demoted to the L0 review queue"
        )
    if not prov.generating_task.strip():
        return False, "L1 provenance: generating_task is required"
    if not [r for r in prov.source_refs if str(r).strip()]:
        return False, "L1 provenance: at least one source_ref (the observation) is required"
    return True, ""


def _gate_for_proposal(provider: Callable[..., "GateSpec"], rec: dict[str, Any]) -> "GateSpec":
    """Call a record-aware gate provider without breaking legacy providers."""
    import inspect
    ...
    return provider(rec) if accepts_record else provider()


def attempt_l1_auto_commit(
    db_path: Path | str,
    store_path: Path | str,
    proposal_id: str,
    *,
    ...
    """Gate-mediated auto-commit. Green: COMMITTED with a full audit record
    (committed_by=gate:auto-L1). Any refusal — admission, provenance,
    frontier, gate red, apply failure — DEMOTES to the L0 human queue with
    the reason recorded; L1 never dead-ends a proposal, it only loses the
    right to commit it."""
    from interaction.write_path_store import WritePathStore
    ...
    try:
        ...
        def _demote(reason: str) -> dict[str, Any]:
            store.update_proposal(proposal_id, status="PENDING", demotion_reason=reason)
            return {"proposal_id": proposal_id, "status": "queued", "claim_level_effective": "L0",
                    "demotion_reason": reason}

        if not policy.l1_admitted:
            return _demote(L1_ADMISSION_INCOMPLETE)
        prov = ProposalProvenance.model_validate(
            {
                "generating_task": rec["generating_task"],
                "source_refs": json.loads(rec["source_refs_json"]),
                "conversation_id": rec["conversation_id"],
                "decision_origin": rec.get("decision_origin", "unspecified"),
            }
        )
        ok, why = _l1_provenance_ok(prov)
        if not ok:
            return _demote(why)
        gap_pred = rec["target_gap_id"]
        is_adjudicative = policy.legislatable is not None and policy.legislatable(gap_pred)
        if is_adjudicative or not policy.frontier_v2_admitted:
            # Reversal-v1 path — unchanged for adjudicative predicates, and
            # the default for everything while v2 is unadmitted.
            ok, why = frontier_check(store, gap_pred, policy)
            if not ok:
                return _demote(why)
            frontier_basis = "recurrence:v1"
        else:
            # Frontier v2, STRUCTURAL predicates only (decision record):
            # demand is proven by the pinned artifact; safety stays with the
            # gate. Purity is REFUSING here (stricter than batch tiering).
            enc = json.loads(rec["encoding_json"])
            pure, purity_findings = structural_purity_check(enc)
            if not pure:
                return _demote("frontier v2: encoding smuggles adjudicative language — "
                               + "; ".join(purity_findings))
            ok, why = verify_artifact_observation(
                policy, json.loads(rec["source_refs_json"]), enc)
            if not ok:
                return _demote(why)
            frontier_basis = "artifact:v2"
        if policy.gate_provider is None:
            return _demote("L1: no gate battery configured on the write policy")
    finally:
        store.close()

    # Commit path reuses the operator machinery — same gates, same snapshot-
    # restore atomicity — with the gate itself as the confirming authority.
    result = confirm_proposal(
        db_path,
        store_path,
        proposal_id,
        primary_source=f"gate:auto-L1 [{policy.admission_evidence or 'admission evidence unset'}] "
        f"[frontier {frontier_basis}] "
        f"observation: {'; '.join(json.loads(WritePathStore(store_path).get_proposal(proposal_id)['source_refs_json']))[:400]}",
        gate=_gate_for_proposal(policy.gate_provider, rec),
        embedder=policy.embedder,
        authority="gate",   # B4: explicit authority, not inferred from the source string
        actor="gate:auto-L1",
    )
    if result.get("status") in ("GATE_FAILED", "ENCODE_FAILED"):
        store = WritePathStore(store_path)
        try:
            reason = f"L1 auto-commit refused: {result['status']} — fell back to human queue"
            store.update_proposal(proposal_id, status="PENDING", demotion_reason=reason)
        finally:
            store.close()
        result["status"] = "queued"
        result["claim_level_effective"] = "L0"
        result["demotion_reason"] = reason
    return result
...
# ---------------------------------------------------------------------------
# L2-3: batch commits + battery tiering (reversal §5, chartered → built)
# ---------------------------------------------------------------------------


def structural_distractor_check(pre_manifest: dict, post_manifest: dict, added_ids: set[str]) -> tuple[bool, list[str]]:
    """Deterministic tier: no pre-existing concept removed or content-changed;
    the only new concepts are the batch's own. Protects structure, not
    semantics — which is exactly why adjudicative writes still take the full
    battery (tiering is a cost policy, never a semantics waiver)."""
    findings: list[str] = []
    pre_c, post_c = pre_manifest["concepts"], post_manifest["concepts"]
    for cid, meta in pre_c.items():
        if cid not in post_c:
            findings.append(f"pre_existing_concept_removed:{cid}")
        elif post_c[cid]["content_sha"] != meta["content_sha"]:
            findings.append(f"pre_existing_concept_mutated:{cid}")
    for cid in post_c.keys() - pre_c.keys():
        if cid not in added_ids:
            findings.append(f"unexpected_concept_added:{cid}")
    return not findings, findings


def _is_adjudicative(prop: SSTProposal, gap_id: str, legislatable: Callable[[str], bool] | None) -> bool:
    if legislatable is not None:
        return bool(legislatable(gap_id))
    return True  # no classifier configured → treat as adjudicative (safe side)
...
def confirm_batch(
    ...
    *,
    primary_source: str,
    gate_builder: Callable[[dict], "GateSpec"],
    embedder: Callable[[str], list[float]] | None = None,
    legislatable: Callable[[str], bool] | None = None,
    require_full_battery_always: bool = False,
    actor: str = "operator",
) -> dict[str, Any]:
    """OPERATOR ONLY. One pin battery per batch instead of per proposal.

    Semantics (pre-registered in run_l23_battery):
    - Sequential apply+closure with incremental snapshots: a closure-red
      proposal is requeued (PENDING, reason=individual closure) and its apply
      is rolled back; the batch continues with survivors.
    - The pin battery runs ONCE at the end over the surviving set. Battery
      red → the WHOLE batch restores to its pre-batch snapshot and every
      survivor demotes with reason "batch distractor regression — split and
      retry individually" (drift is attributable only per-proposal; the
      honest batch response is to un-batch).
    - Tiering: an all-structural batch (per `legislatable`) takes the
      deterministic structural tier; any adjudicative proposal forces the
      full semantic battery. `require_full_battery_always=True` disables the
      cheap tier. The tier is recorded per proposal (`gate_tier`).
    - Every gate spec in the batch must agree on pin config (same baseline
      + pin ids); disagreement is a typed refusal — pins are corpus-level.
    - Batch requires split runners on the specs (closure_runner/pins_runner);
      legacy single-runner specs are a typed refusal.
    """
    from interaction.write_path_store import WritePathStore
    from interaction.event_log import latest_event_id
    from mcp_server.history import SnapshotStore, extract_manifest
    ...
    db_path = Path(db_path)
    if not str(primary_source).strip():
        return {"error": "primary_source is required for the batch"}
    if not proposal_ids:
    ...
    try:
        ...
        for pid in proposal_ids:
            ...
            if rec is None:
                return {"error": f"unknown proposal: {pid}"}
            if rec["status"] != "PENDING":
                return {"error": f"{pid} is {rec['status']}, not PENDING"}
            basis_refusal = proposal_basis_refusal(rec, db_path)
            if basis_refusal is not None:
                return {
                    **basis_refusal,
                    "error": f"{pid}: {basis_refusal['error']}",
                }
            prop, err = validate_proposal(json.loads(rec["encoding_json"]), db_path)
            if prop is None:
                return {"error": f"{pid} no longer valid: {err}"}
            spec = gate_builder(rec)
            if spec.closure_runner is None or spec.pins_runner is None:
                return {"error": f"{pid}: batch commits require split runners "
                                 "(closure_runner + pins_runner) on the GateSpec"}
            recs.append(rec)
            specs.append(spec)
            props.append(prop)

        pin_fingerprint = {(json.dumps(sp.baseline, sort_keys=True),
                            sp.gap_anchor_ids, sp.intrinsic_ids, sp.flaky_anchor_ids)
                           for sp in specs}
        if len(pin_fingerprint) > 1:
            return {"error": "pin configuration differs across the batch — pins are "
                             "corpus-level; split the batch by corpus"}

        adjudicative = any(
            _is_adjudicative(prop, rec["target_gap_id"], legislatable)
            for prop, rec in zip(props, recs)
        ) or require_full_battery_always
        purity_note = None
        if not adjudicative:
            # Hardening: the cheap tier is not reachable by NAME alone —
            # smuggled adjudicative language in any concept's content forces
            # the full battery (recorded in results for the audit).
            for rec in recs:
                pure, purity_findings = structural_purity_check(json.loads(rec["encoding_json"]))
                if not pure:
                    adjudicative = True
                    purity_note = {"forced_full_tier_by": rec["proposal_id"],
                                   "purity_findings": purity_findings}
                    break
        tier = "full" if adjudicative else "structural"

        snaps = SnapshotStore(db_path)
        batch_id = f"batch_{uuid.uuid4().hex[:10]}"
        batch_label = f"pre-batch:{batch_id}"
        snaps.capture(batch_label)
        pre_manifest = extract_manifest(db_path)
        emb = embedder or default_embedder()

        results: dict[str, Any] = {}
        if purity_note:
            results["_tier_note"] = purity_note
        survivors: list[tuple[dict, "GateSpec", SSTProposal]] = []
        for rec, spec, prop in zip(recs, specs, props):
            pid = rec["proposal_id"]
            step_label = f"pre-step:{batch_id}:{pid}"
            snaps.capture(step_label)
            try:
                _apply(db_path, prop, emb)
            except Exception as exc:
                snaps.restore(step_label)
                store.update_proposal(pid, status="PENDING", batch_id=batch_id, gate_tier=tier,
                                      demotion_reason=f"batch apply failed: {type(exc).__name__}")
                results[pid] = {"status": "queued", "reason": "apply_failed"}
                continue
            closure = check_closure(
                spec.target_gap_id, spec.closure_runner(),
                policy_id=spec.policy_id, also_valid=spec.also_valid,
                wrong_adjacent=spec.wrong_adjacent,
                policy_in_grounding=spec.policy_in_grounding,
                adjacent_only=spec.adjacent_only, min_gov_rate=spec.min_gov_rate,
            )
            if not closure.get("closes_cleanly"):
                snaps.restore(step_label)
                store.update_proposal(pid, status="PENDING", batch_id=batch_id, gate_tier=tier,
                                      demotion_reason="batch: individual closure red — requeued",
                                      gate_report_json=json.dumps({"closure": closure}))
                results[pid] = {"status": "queued", "reason": "closure_red"}
                continue
            survivors.append((rec, spec, prop))
            results[pid] = {"status": "closure_green", "closure": closure}

        if survivors:
            if tier == "full":
                post_rows = survivors[0][1].pins_runner()  # pins are shared — one run
                clean, findings = check_distractors(
                    survivors[0][1].baseline, post_rows,
                    encoded_gap_ids=[r["target_gap_id"] for r, _, _ in survivors],
                    gap_anchor_ids=survivors[0][1].gap_anchor_ids,
                    flaky_anchor_ids=survivors[0][1].flaky_anchor_ids,
                    intentional_closure_ids=tuple(r["target_gap_id"] for r, _, _ in survivors),
                    intrinsic_ids=survivors[0][1].intrinsic_ids,
                )
                findings_dump = [f.model_dump() if hasattr(f, "model_dump") else dict(f) for f in findings]
            else:
                added = {c.id for _, _, prop in survivors for c in prop.concepts}
                clean, s_findings = structural_distractor_check(
                    pre_manifest, extract_manifest(db_path), added)
                findings_dump = [{"kind": f} for f in s_findings]

            if clean:
                post_label = f"post-batch:{batch_id}"
                snaps.capture(post_label)
                for rec, _, prop in survivors:
                    cause = latest_event_id(store_path, proposal_id=rec["proposal_id"])
                    subject_ids, commit_payload = _commit_subjects(prop)
                    _emit(store_path, type=event_types.GRAPH_COMMITTED, proposal_id=rec["proposal_id"],
                          gap_id=rec["target_gap_id"], batch_id=batch_id,
                          graph_version_before=batch_label, graph_version_after=post_label,
                          actor=actor, authority_type="human",
                          causation_event_id=cause or "",
                          subject_node_ids=subject_ids,
                          payload=commit_payload)
                    store.update_proposal(
                        rec["proposal_id"], status="COMMITTED", batch_id=batch_id,
                        gate_tier=tier, primary_source=primary_source,
                        gate_report_json=json.dumps(
                            {"closure": results[rec["proposal_id"]]["closure"],
                             "distractors_clean": True, "tier": tier,
                             "findings": findings_dump}),
                        graph_version_before=batch_label, graph_version_after=post_label,
                        decided_at=time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()))
                    results[rec["proposal_id"]] = {"status": "COMMITTED"}
            else:
                snaps.restore(batch_label)
                for rec, _, _ in survivors:
                    store.update_proposal(
                        rec["proposal_id"], status="PENDING", batch_id=batch_id, gate_tier=tier,
                        demotion_reason="batch distractor regression — split and retry individually",
                        gate_report_json=json.dumps({"distractors_clean": False, "tier": tier,
                                                     "findings": findings_dump}))
                    results[rec["proposal_id"]] = {"status": "queued", "reason": "batch_distractor_red"}

        return {"batch_id": batch_id, "tier": tier, "results": results}
    finally:
        store.close()


def structural_closure_rows(db_path: Path | str, gap_id: str, enc: dict) -> list[dict]:
    """Shared deterministic closure for STRUCTURAL proposals (L2-4: factored
    out of the M6 gate): GOVERNED iff every proposed concept and edge is
    present in the live manifest; the grounding states the structural fact
    and carries the gap id so right-reason passes on truth."""
    from mcp_server.history import extract_manifest

    manifest = extract_manifest(Path(db_path))
    concept_ids = [c.get("id", "") for c in (enc.get("concepts") or [])]
    missing_c = [cid for cid in concept_ids if cid not in manifest["concepts"]]
    edge_set = {tuple(e) for e in manifest["edges"]}
    missing_e = [
        [e["type"], e["source_id"], e["target_id"], e.get("label", "") or ""]
        for e in (enc.get("edges") or [])
        if (e["type"], e["source_id"], e["target_id"], e.get("label", "") or "") not in edge_set
    ]
    ok = not missing_c and not missing_e
    grounding = (
        f"{gap_id} structural closure: concepts_present={concept_ids if ok else 'PARTIAL'} "
        f"missing_concepts={missing_c} missing_edges={missing_e}"
    )
    return [{"governance_verdict": "GOVERNED" if ok else "UNGOVERNED", "grounding": grounding}]


def verify_artifact_observation(
    policy: WritePolicy, source_refs: list[str], encoding: dict
) -> tuple[bool, str]:
    """Frontier v2 structural demand check: "the code is the recurrence."
    Every requirement is deterministic and reads ONLY the operator-pinned
    trust surface. Absence of evidence refuses (a missing manifest is not a
    pass). Returns (ok, typed_reason)."""
    if policy.artifact_root is None:
        return False, "frontier v2: no operator-pinned artifact root on the write policy"
    root = Path(policy.artifact_root).resolve()
    refs = [r for r in source_refs if str(r).startswith("artifact://")]
    if not refs:
        return False, "frontier v2: no artifact:// source_ref (structural demand is proven by the artifact)"
    if policy.artifact_test_manifest is None or not Path(policy.artifact_test_manifest).exists():
        return False, "frontier v2: no pinned test manifest (absence is not a pass)"
    manifest = json.loads(Path(policy.artifact_test_manifest).read_text())

    def _squash(t: str) -> str:
        return "".join(ch for ch in t.lower() if ch.isalnum())

    idents = [c.get("id", "") for c in encoding.get("concepts") or []] + [
        c.get("label", "") for c in encoding.get("concepts") or []
    ]
    ...
    for ref in refs:
        ...
        if root not in target.parents and target != root:
            return False, f"frontier v2: {rel!r} escapes the pinned root"
        if not target.is_file():
            return False, f"frontier v2: artifact {rel!r} not found under the pinned root"
        text = target.read_text(errors="replace")
        squashed = _squash(text)
        if not any(i.lower() in text.lower() or _squash(i) in squashed for i in idents):
            return False, (f"frontier v2: artifact {rel!r} references no proposed identifier "
                           "(the code must evidence the structure it claims)")
        status = str(manifest.get(rel, "")).lower()
        if status != "pass":
            return False, (f"frontier v2: pinned manifest marks {rel!r} as "
                           f"{'failing' if status == 'fail' else 'unattested'} — refused")
    return True, ""

# ---------------------------------------------------------------------------
# Structural purity (hardening, decision-independent): the batch structural
# tier and any future structural-eligibility path must not be reachable by
# NAME alone — a "structural" proposal smuggling adjudicative language in its
# content is adjudicative, and takes the full battery.
# ---------------------------------------------------------------------------

_ADJUDICATIVE_MARKERS = (
    "adjudicates", "must not", "must ", "shall ", "is a violation",
    "violations", "prohibited", "forbidden", "governs ", "is governed",
    "policy:", "does not govern",
)


def structural_purity_check(encoding: dict) -> tuple[bool, list[str]]:
    """(pure, findings). A proposal is structurally pure when no concept's
    text/anchor/label carries adjudicative language. Deterministic and
    conservative: markers err toward impurity — an impure proposal is not
    refused anywhere, it is merely denied the CHEAP tier."""
    findings: list[str] = []
    for c in encoding.get("concepts") or []:
        blob = " ".join(str(c.get(k, "")) for k in ("label", "text_content", "semantic_anchor")).lower()
        hits = sorted({m.strip() for m in _ADJUDICATIVE_MARKERS if m in blob})
        if hits:
            findings.append(f"adjudicative_language_in:{c.get('id', '?')}:{','.join(hits)}")
    return not findings, findings
